...
export default {
  ...
  methods: {
    submit() {
      ...
      axios.put(this.$apiAddress + '/questions/' + this.$route.params.id, formData).then(r => {
        console.log(r.data)
        this.$router.push('/')
      })
    },
    fetchPertanyaan() {
      axios.get(this.$apiAddress + '/questions/' + this.$route.params.id).then(r => {
        const { question, answer, choices, topic } = r.data.data
